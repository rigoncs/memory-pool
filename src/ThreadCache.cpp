...
namespace Rigon_memoryPool {
    ...
    void* ThreadCache::allocate(size_t size) {
        ...
        if (void* ptr = freeList_[index]) {
            freeList_[index] = *reinterpret_cast<void**>(ptr); // 将 freeList_[index]指向的内存块的下一个内存块地址
            return ptr;
        }

        // 如果本地线程自由链表为空，则从中心缓存获取一批内存
    ...
    size_t ThreadCache::getBatchNum(size_t size) {
        ...
        else baseNum = 1; // 大于1024的对象每次只从中心缓存获取1个
