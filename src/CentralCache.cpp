...
namespace Rigon_memoryPool {
                ...
                if (blockNum > 1) {
                    ...
                    *reinterpret_cast<void**>(result) = nullptr;
                    ...
                    if (trackerIndex < spanTrackers_.size()) {
                        ...
                        spanTrackers_[trackerIndex].freeCount.store(blockNum-1, std::memory_order_release); // 第一个result已经被分配出去，所以初始空闲块数为blockNum-1
                    }
                }
            } else {
                // 保存result的下一个节点
    ...
    void CentralCache::returnRange(void *start, size_t size, size_t index) {
        ...
        try {
            ...
            *reinterpret_cast<void**>(end) = current; // 头插法(将原有链表接在归还链表后面)
